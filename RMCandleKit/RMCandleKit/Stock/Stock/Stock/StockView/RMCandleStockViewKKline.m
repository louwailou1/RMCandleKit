...
#import "RMCandleKlineMaskView.h"
@interface RMCandleStockViewKKline() <UIScrollViewDelegate>
...
/**
 数据源
 */
@property (nonatomic, strong) NSArray <id<RMCandleLineDataModelProtocol>>*lineModels;

/**
 K线部分
 */
@property (nonatomic, copy) RMCandleKlineView *kLineView;

/**
 成交量部分
 */
@property (nonatomic, copy) RMCandleKlineVolumeView *volumeView;

/**
 当前绘制在屏幕上的数据源数组
 */
@property (nonatomic, strong) NSMutableArray <id<RMCandleLineDataModelProtocol>>*drawLineModels;

/**
 当前绘制在屏幕上的数据源位置数组
 */
@property (nonatomic, copy) NSArray <RMCandleLinePositionModel *>*drawLinePositionModels;

/**
 长按时出现的遮罩View
 */
@property (nonatomic, strong) RMCandleKlineMaskView *maskView;

@end

@implementation RMCandleStockViewKKline {
...
#pragma mark - 页面上显示的数据
    ...
    //当前长按选中的model
    id<RMCandleLineDataModelProtocol> selectedModel;
}
...
//    [RMCandleStockVariable setLineWith:(self.stockScrollView.bounds.size.width / lineModels.count) - [RMCandleStockVariable lineGap]];
    [self layoutIfNeeded];
...
- (void)initUI {
    ...
    //加载LineView
    _kLineView = [RMCandleKlineView new];
    ...
    //加载VolumeView
    _volumeView = [RMCandleKlineVolumeView new];
    ...
    [_volumeView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(_stockScrollView.contentView).multipliedBy([RMCandleStockVariable volumeViewRadio]);
    }];
...
- (void)drawLeftDesc {
    ...
    CGFloat unit = self.stockScrollView.frame.size.height * [RMCandleStockVariable lineMainViewRadio] / 4.f;
    ...
    CGFloat kTextY = YYStockScrollViewTopGap + self.stockScrollView.frame.size.height * (1 - [RMCandleStockVariable volumeViewRadio])+10;
    CGFloat wanshouY = YYStockScrollViewTopGap + 15 + self.stockScrollView.frame.size.height * (1 - [RMCandleStockVariable volumeViewRadio]) +10 ;
    ...
    if (wVolume > 1) {
        ...
        if (yVolume > 1) {
            ...
            [text drawInRect:CGRectMake(YYStockScrollViewLeftGap - textSize.width - 5, kTextY, 60, 20) withAttributes:attribute];
            
            NSString *descText = @"亿手";
            CGSize textSize1 = [self rectOfNSString:descText attribute:attribute].size;
            [descText drawInRect:CGRectMake(YYStockScrollViewLeftGap - textSize1.width - 5, wanshouY, 60, 20) withAttributes:attribute];
        } else {
...
- (void)drawTopDesc {
    ...
    NSDictionary *attribute = @{NSFontAttributeName:[UIFont systemFontOfSize:9],NSForegroundColorAttributeName:[UIColor YYStock_topBarNormalTextColor]};
    NSString *ma5Text = [NSString stringWithFormat:[NSString stringWithFormat:@"MA5 %@", RMCandleConStant.sharedInstance.accuracy],[selectedModel MA5]];
    NSString *ma10Text = [NSString stringWithFormat:[NSString stringWithFormat:@"MA10 %@", RMCandleConStant.sharedInstance.accuracy],[selectedModel MA10]];
    NSString *ma20Text = [NSString stringWithFormat:[NSString stringWithFormat:@"MA20 %@", RMCandleConStant.sharedInstance.accuracy],[selectedModel MA20]];
...
- (void)updateDrawModels {
    ...
    NSInteger drawLineCount = (self.stockScrollView.frame.size.width) / ([RMCandleStockVariable lineGap] +  [RMCandleStockVariable lineWidth]);
//    NSInteger drawLineCount = (self.stockScrollView.frame.size.width - [RMCandleStockVariable lineGap]) / ([RMCandleStockVariable lineGap] +  [RMCandleStockVariable lineWidth]);
    ...
    [self.drawLineModels enumerateObjectsUsingBlock:^(id<RMCandleLineDataModelProtocol>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSInteger)xPosition {
    ...
    CGFloat startXPosition = (leftArrCount + 1) * [RMCandleStockVariable lineGap] + leftArrCount * [RMCandleStockVariable lineWidth] + [RMCandleStockVariable lineWidth]/2;
...
//    NSUInteger leftCount = ABS(offsetX - [RMCandleStockVariable lineGap]) / ([RMCandleStockVariable lineGap] + [RMCandleStockVariable lineWidth]);
//    NSUInteger leftCount = ceilf((offsetX - [RMCandleStockVariable lineGap]) / ([RMCandleStockVariable lineGap] + [RMCandleStockVariable lineWidth]));
    NSUInteger leftCount = ABS(offsetX) / ([RMCandleStockVariable lineGap] + [RMCandleStockVariable lineWidth]);
...
- (CGFloat)updateScrollViewContentWidth {
    ...
    CGFloat kLineViewWidth = self.lineModels.count * [RMCandleStockVariable lineWidth] + (self.lineModels.count + 1) * [RMCandleStockVariable lineGap];
...
- (void)event_longPressAction:(UILongPressGestureRecognizer *)longPress {
    ...
    if(UIGestureRecognizerStateChanged == longPress.state || UIGestureRecognizerStateBegan == longPress.state) {
        ...
        NSInteger startIndex = (NSInteger)((oldPositionX - [self xPosition] + ([RMCandleStockVariable lineGap] + [RMCandleStockVariable lineWidth])/2.f) / ([RMCandleStockVariable lineGap] + [RMCandleStockVariable lineWidth]));
        ...
        if (!self.maskView) {
            _maskView = [RMCandleKlineMaskView new];
...
- (void)event_pinchAction:(UIPinchGestureRecognizer *)pinch {
    ...
    if(ABS(difValue) > YYStockLineScaleBound) {
        if( pinch.numberOfTouches == 2 ) {
            ...
            //3.拿到中心点数据源的index
            CGFloat oldLeftArrCount = ABS(centerX + [RMCandleStockVariable lineGap]) / ([RMCandleStockVariable lineGap] + [RMCandleStockVariable lineWidth]);
            
            //4.缩放重绘
            CGFloat newLineWidth = [RMCandleStockVariable lineWidth] * (difValue > 0 ? (1 + YYStockLineScaleFactor) : (1 - YYStockLineScaleFactor));
            [RMCandleStockVariable setLineWith:newLineWidth];
            [self updateScrollViewContentWidth];
            
            //5.计算更新宽度后捏合中心点距离klineView左侧的距离
            CGFloat newLeftDistance = oldLeftArrCount * [RMCandleStockVariable lineWidth] + (oldLeftArrCount - 1) * [RMCandleStockVariable lineGap];
            
            //6.设置scrollview的contentoffset = (5) - (2);
            if ( self.lineModels.count * newLineWidth + (self.lineModels.count + 1) * [RMCandleStockVariable lineGap] > self.stockScrollView.bounds.size.width ) {
